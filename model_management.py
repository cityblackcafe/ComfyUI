"""
    This file is part of FuxkComfy.
...
try:
    from comfy.ultra_high_speed_bridge import (
        is_ultra_high_speed_enabled,
        get_transfer_bandwidth,
        cpu_to_gpu_ultra,
        gpu_to_cpu_ultra
    )
    _ultra_bridge_available = True
except ImportError:
    _ultra_bridge_available = False
...
def minimum_inference_memory():
    return (1024 * 1024 * 1024) * 1.5 + extra_reserved_memory()
...
def maximum_vram_for_weights(device=None):
    return (get_total_memory(device) * 0.75 - minimum_inference_memory())
...
_ultra_bridge_stats = {'used': 0, 'fallback': 0, 'total': 0}

def cast_to(weight, dtype=None, device=None, non_blocking=False, copy=False, stream=None):
    global _ultra_bridge_stats
    ...
    _ultra_bridge_stats['total'] += 1

    if is_ultra_high_speed_enabled() and stream is None and not copy:
        try:
            if device is not None and str(device).startswith('cuda'):
                if not weight.is_cuda:
                    _ultra_bridge_stats['used'] += 1
                    if _ultra_bridge_stats['used'] <= 3:
                        size_mb = weight.numel() * weight.element_size() / (1024 * 1024)
                        logging.info(f"[Ultra Bridge] CPU->GPU transfer: {size_mb:.1f} MB")
                    result = cpu_to_gpu_ultra(weight, device)
                    if dtype is not None and result.dtype != dtype:
                        result = result.to(dtype=dtype)
                    return result
                elif weight.is_cuda and device != weight.device:
                    result = weight.to(device=device, non_blocking=non_blocking)
                    if dtype is not None and result.dtype != dtype:
                        result = result.to(dtype=dtype)
                    return result
        except Exception as e:
            _ultra_bridge_stats['fallback'] += 1
            logging.debug(f"Ultra high-speed transfer failed, using standard: {e}")
...
def get_ultra_bridge_stats():
    return _ultra_bridge_stats.copy()
